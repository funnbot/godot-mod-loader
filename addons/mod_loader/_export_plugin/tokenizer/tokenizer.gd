...
var _pending_comment: bool = false;
var _comment: String = "";

var _pending_whitespace: bool = false;
var _whitespace: String = "";
...
func _make_error(message: String) -> Token:
	...
	var token := Token.new(Type.ERROR);
...
func _number() -> Token:
	...
	if base != 10 and ch == Codepoint.UNDERSCORE:
		_make_error("Unexpected underscore after 0%c" % _peek(-1));
		has_error = true;
	...
	while true:
		...
		if _current_char == Codepoint.UNDERSCORE:
			if previous_was_underscore:
				_make_error("Multiple underscores cannot be adjacent in a numeric literal.");
				# not a fatal error
	...
	if ch == Codepoint.PERIOD and _peek(1) != Codepoint.PERIOD:
		if base == 10 and not has_decimal:
			...
			_make_error("Cannot use a decimal point twice in a _number.");
			has_error = true;
		elif base == 16:
			_make_error("Cannot use a decimal point in a hexadecimal _number.");
			has_error = true;
		else:
			_make_error("Cannot use a decimal point in a binary _number.");
			has_error = true;
		if not has_error:
			_advance();
		...
		if not has_error:
			...
			if Codepoint.UNDERSCORE == ch:
				_make_error("Unexpected underscore after decimal point.");
				has_error = true;
			previous_was_underscore = false;
	...
	if base == 10 and (_current_char == Codepoint.LO_E or _current_char == Codepoint.UP_E):
		...
		if not Codepoint.is_digit(_current_char):
			_make_error("Expected digit after exponent.");
		previous_was_underscore = false;
	...
	if need_digits:
		_make_error("Expected %s digit after \"0%c\"" %
	...
	if not has_error and has_decimal and _current_char == Codepoint.PERIOD and _peek(1) != Codepoint.PERIOD:
		...
		_make_error("Invalid numeric notation.");
...
func _string() -> Token:
	...
	while true:
		...
		if Codepoint.is_invisible_direction_control(_current_char):
			if is_raw:
				_make_error("Invisible text direction control character present in the _string, use regular _string literal instead of r-_string.");
			else:
				_make_error("Invisible text direction control character present in the _string, escape it (\"\\u%s\") to avoid confusion."
		...
		if _current_char == Codepoint.BACKSLASH:
			...
			match escape_char:
				...
				Codepoint.UP_U, Codepoint.LO_U:
					...
					for i in range(hex_len):
						...
						if Codepoint.is_digit(digit):
							...
							_make_error("Invalid hexadecimal digit in unicode escape sequence");
							valid_escape = false;
				...
				Codepoint.CARRIAGE_RETURN, Codepoint.LINE_FEED:
					...
					valid_escape = false;
				_:
					_make_error("Invalid escape in _string.");
					valid_escape = false;
			...
			if valid_escape:
				if (escaped & 0xfffffc00) == 0xd800:
					if prev_char == Codepoint.NULL:
						...
						_make_error("Invalid UTF-16 sequence in _string, unpaired lead surrogate.");
						valid_escape = false;
						prev_char = Codepoint.NULL;
				elif (escaped & 0xfffffc00) == 0xdc00:
					if prev_char == Codepoint.NULL:
						_make_error("Invalid UTF-16 sequence in _string, unpaired trail surrogate.");
						valid_escape = false;
					else:
...
func _check_indent() -> void:
	...
	while true:
		...
		if _current_char == Codepoint.CARRIAGE_RETURN:
			_advance();
			if _current_char != Codepoint.LINE_FEED:
				_make_error("Stray carriage return character in source code.");
		if _current_char == Codepoint.LINE_FEED:
		...
		if mixed and not _line_continuation and not _multiline_mode:
			_make_error("Mixed indentation (spaces and tabs) not allowed.");
		...
		if _indent_char == Codepoint.NULL:
			...
			_make_error("Used %s character for indentation instead of %s as used before in the file." %
		...
		if indent_count > previous_indent:
			...
			# indent decrease
			if _indent_level() == 0:
				_make_error("Tokenizer bug: trying to dedent without previous indent.");
				return ;
			...
			if (_indent_level() > 0 and _indent_stack.a.back() != indent_count) or (_indent_level() == 0 and indent_count != 0):
				_make_error("Unindent doesn't match any outer indentation level.");
	...
	match _consumed_char:
		...
		_:
			if Codepoint.is_whitespace(_consumed_char):
				return _make_error("Invalid white space character U+%04X." % _consumed_char);
			else:
